! function($) {
	...
	BootstrapPaginator.prototype = {
		...
		setOptions: function(options) {
			this.options = $.extend(
				...
				this.listen(), 
				this.render(), 
				this.initialized || this.lastPage === this.currentPage || this.$element.trigger("page-changed", 
		...
		listen: function() {
			...
			this.options.onPageClicked), "function" == typeof this.options.onPageChanged && this.$element.on("page-changed", 
			this.options.onPageChanged), this.$element.bind("page-clicked", 
			this.onPageClicked)
		},
		destroy: function() {
			this.$element.off("page-clicked"), 
		...
		show: function(page) {
			...
			this.render(), 
			this.lastPage !== 
			this.currentPage && this.$element.trigger("page-changed", [this.lastPage, this.currentPage])
		},
		showNext: function() {
	...
	//1- return undefined
	$.fn.bootstrapPaginator = function(option) {//首先运行这一个
		var args = arguments;
		var result = null;
		...
		$(this).each(function(index, item) {
			//获取调用此方法的节点 是$节点
			var $this = $(item);
			//获得此节点的bootstrapPaginator的值
			var data = $this.data("bootstrapPaginator");
			var options = "object" != typeof option ? null : option;
			//如果data没有数据执行	最开始调用的ul都没有data-bootstrapPaginator
			if(!data) {
				data = new BootstrapPaginator(this, options);
				$this = $(data.$element);
				$this.data("bootstrapPaginator", data);
				//就是return undefined的意思
				return void 0;
			}
			//如果传入的参数是string
